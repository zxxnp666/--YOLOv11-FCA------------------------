...
class FoggyTrafficSystem(nn.Module):
    ...
    将DEA-Net去雾模块和YOLOv11-FCA检测模块集成为端到端系统
    
    Args:
        deanet_weights: DEA-Net预训练权重路径
    ...
    def __init__(self, deanet_weights=None, yolov11_weights=None, 
        ...
        # 初始化检测模块
        self.yolov11_fca = YOLOv11FCA(num_classes=num_classes)
        if yolov11_weights:
            self.yolov11_fca.load_pretrained(yolov11_weights)
    ...
    def predict(self, hazy_image):
        ...
        Args:
            hazy_image: 雾天图像, shape [B, 3, H, W] 或 numpy array
        ...
        with torch.no_grad():
            detections, dehazed_image = self.forward(hazy_image, return_dehazed=True)
